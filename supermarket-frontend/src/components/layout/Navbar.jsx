  ...
  return (
    <>
      <style>{`
        ...
        .nb-admin{padding:8px 14px;border-radius:12px;background:linear-gradient(135deg,#7c3aed,#8b5cf6);color:#fff;font-family:'Plus Jakarta Sans',sans-serif;font-size:12px;font-weight:700;text-decoration:none;transition:all .2s;display:flex;align-items:center;gap:5px}
      ...
      <div style={{background:'linear-gradient(90deg,#14532d,#15803d,#14532d)',padding:'7px 0'}}>
        <div style={{maxWidth:1280,margin:'0 auto',padding:'0 24px',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
          <span style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:12,color:'rgba(255,255,255,.75)'}}>
            📍 Phnom Penh, Cambodia &nbsp;|&nbsp; +855 12 345 678
          </span>
          <div style={{display:'flex',gap:16,alignItems:'center'}}>
            {user ? (
              <>
                <span style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:12,color:'#86efac'}}>
                  👋 {user.name?.split(' ')[0] || user.email}
                  {ADMIN_ROLES.includes(user.role) && (
                    <span style={{marginLeft:6,background:'rgba(124,58,237,.3)',padding:'1px 8px',borderRadius:99,fontSize:11,color:'#c4b5fd'}}>
                      {user.role}
                    </span>
                  )}
                </span>
                <button onClick={logoutUser} style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:12,color:'rgba(255,255,255,.65)',background:'none',border:'none',cursor:'pointer'}}>
                  Sign Out
                </button>
              </>
            ) : (
      ...
      <header style={{position:'sticky',top:0,zIndex:100,background:scrolled?'rgba(255,255,255,.97)':'#fff',backdropFilter:scrolled?'blur(16px)':'none',borderBottom:'1px solid #e5e7eb',boxShadow:scrolled?'0 4px 24px rgba(0,0,0,.07)':'none',transition:'all .3s'}}>
        <div style={{maxWidth:1280,margin:'0 auto',padding:'0 24px',height:70,display:'flex',alignItems:'center',gap:20}}>
          ...
          <Link to="/" style={{textDecoration:'none',display:'flex',alignItems:'center',gap:10,flexShrink:0}}>
            ...
            <div>
              <div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontWeight:800,fontSize:20,color:'#111',lineHeight:1.1}}>
          ...
          <div style={{display:'flex',alignItems:'center',gap:8,flexShrink:0}}>
            <Link to="/wishlist" className="nb-icon" style={{background:'linear-gradient(135deg,#fce7f3,#fbcfe8)',borderColor:'#f9a8d4'}}>❤️</Link>
            <Link to="/cart" className="nb-icon" style={{background:'linear-gradient(135deg,#fef9c3,#fef08a)',borderColor:'#fde047'}}>
            ...
            {user ? (
              <div style={{display:'flex',gap:7,alignItems:'center'}}>
                ...
                {/* ✅ Show Admin button for BOTH admin and manager */}
                {ADMIN_ROLES.includes(user.role) && (
        ...
        {/* CATEGORY BAR */}
        <div style={{background:'linear-gradient(90deg,#14532d,#166534,#15803d)'}}>
          <div className="nb-catbar" style={{maxWidth:1280,margin:'0 auto',padding:'0 24px',display:'flex',alignItems:'center',gap:2}}>
            {categories.length > 0
              ? categories.map(cat=>(
                  <Link key={cat.Category_ID} to={`/shop?cat=${encodeURIComponent(cat.Category_Name)}`} className="nb-cat">
                    {EMOJI_MAP[cat.Category_Name]||'📦'} {cat.Category_Name}
                  </Link>
                ))
              : ['Produce','Dairy','Meat','Bakery','Beverages','Snacks'].map(c=>(
                  <Link key={c} to={`/shop?cat=${c}`} className="nb-cat">
                    {EMOJI_MAP[c]||'📦'} {c}
                  </Link>
                ))
            }
            <div style={{marginLeft:'auto',fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:12,color:'rgba(255,255,255,.7)',padding:'10px 0',whiteSpace:'nowrap',flexShrink:0}}>
