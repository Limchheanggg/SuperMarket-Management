...
// 1 point per $1 spent, tiers based on total spent
...
export default function Dashboard() {
  ...
  const fetchAll = async () => {
    ...
    try {
      ...
      const totalSpent  = allSales.reduce((s,o) => s + Number(o.Total_Amount||0), 0)
      // 1 loyalty point per $1 spent — auto calculated
      const autoPoints  = Math.floor(totalSpent)
      const autoTier    = getTier(totalSpent)
    ...
    // ── Inventory (for admin/manager alert) ─────────────────────────────
    try {
      const invRes = await API.get('/api/inventory/').catch(() => ({ data:[] }))
      setInventory(invRes.data || [])
    } catch {}

    // ── Membership override — if registered, use real membership points ──
    // Only for customers (admin/manager get auto-calculated points above)
    if (!['admin','manager'].includes(user?.role)) {
      try {
        const memRes = await API.get('/api/membership/').catch(() => ({ data:[] }))
        const members = memRes.data || []
        const myMembership = members.find(m =>
          m.full_name?.toLowerCase() === name.toLowerCase() ||
          m.full_name?.toLowerCase().includes((name.split(' ')[0] || '').toLowerCase())
        )
        if (myMembership) {
          setStats(prev => ({
            ...prev,
            points: myMembership.points || Math.floor(prev.spent),
        ...
        <div style={{ display:'grid', gridTemplateColumns:'220px 1fr', minHeight:'70vh', background:'#fff', borderRadius:18, overflow:'hidden', border:'1.5px solid #e5e7eb', boxShadow:'0 4px 20px rgba(0,0,0,.06)' }}>
          ...
          <aside style={{ background:'linear-gradient(180deg,#0f172a,#1e293b)', padding:'28px 12px', display:'flex', flexDirection:'column' }}>
            <div style={{ padding:'0 8px 24px', borderBottom:'1px solid rgba(255,255,255,.07)', marginBottom:12 }}>
              <div style={{ width:52, height:52, borderRadius:15, background:`linear-gradient(135deg,${tierColor},${tierColor}88)`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:22, fontWeight:800, color:'#fff', marginBottom:10 }}>
                {name[0]?.toUpperCase()}
              </div>
          ...
          <div style={{ padding:28, background:'#f8fafc' }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:14, marginBottom:24 }}>
              {[
                ['Total Orders',   stats.orders,                 '#6366f1', 'linear-gradient(135deg,#eef2ff,#e0e7ff)'],
                ['Loyalty Points', `${stats.points} pts`,        tierColor, `linear-gradient(135deg,${tierColor}15,${tierColor}08)`],
                ['Total Spent',    `$${stats.spent.toFixed(2)}`, '#db2777', 'linear-gradient(135deg,#fdf2f8,#fce7f3)'],
              ].map(([l,v,c,bg]) => (
            ...
            {/* Recent Orders */}
            <div style={{ background:'#fff', borderRadius:14, padding:20, border:'1.5px solid #e5e7eb', marginBottom:16 }}>
              ...
              {loading ? (
                <div style={{ textAlign:'center', color:'#94a3b8', padding:20, fontSize:13 }}>Loading…</div>
                ...
                <div key={i} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'9px 0', borderBottom:'1px solid #f8fafc', fontSize:13 }}>
                  <span style={{ color:'#16a34a', fontWeight:700 }}>{o.Sale_ID}</span>
                  <span style={{ color:'#6b7280', fontSize:12 }}>{o.date}</span>
                  <span style={{ padding:'3px 10px', borderRadius:99, fontSize:11, fontWeight:700, background:'#dcfce7', color:'#15803d' }}>{o.status}</span>
            ...
            <div style={{ background:'#fff', borderRadius:14, padding:20, border:'1.5px solid #e5e7eb', marginBottom:16 }}>
              ...
              <div style={{ background:`linear-gradient(135deg,${tierColor}15,${tierColor}08)`, borderRadius:12, padding:16, marginBottom:14, border:`1px solid ${tierColor}33` }}>
                <div style={{ display:'flex', alignItems:'center', gap:12 }}>
                  <span style={{ fontSize:36 }}>{tierIcon}</span>
                  <div>
                    <div style={{ fontSize:28, fontWeight:800, color:tierColor, fontFamily:"'Plus Jakarta Sans',sans-serif", lineHeight:1 }}>{stats.points} pts</div>
                    <div style={{ fontSize:13, color:'#64748b', marginTop:2 }}>
                      {stats.tier} Member · ${stats.spent.toFixed(2)} spent · <strong style={{ color:tierColor }}>1 pt per $1</strong>
                    </div>
                  </div>
                </div>
              </div>
              <div style={{ background:'#e5e7eb', height:8, borderRadius:99, overflow:'hidden', marginBottom:6 }}>
                <div style={{ background:`linear-gradient(90deg,${tierColor},${tierColor}88)`, height:'100%', width:`${Math.max(progress,2)}%`, borderRadius:99, transition:'width .5s' }} />
              </div>
              <div style={{ display:'flex', justifyContent:'space-between', fontSize:11, color:'#94a3b8', marginBottom:16 }}>
                <span>{tierIcon} {stats.tier} (${TIER_THRESHOLDS[stats.tier]})</span>
                {nextTier
                  ? <span>{TIER_ICONS[nextTier]} {nextTier} — ${Math.max(0, nextVal - stats.spent).toFixed(2)} away</span>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:12, marginBottom:16 }}>
              {[
                ['🛒 Browse Shop','/shop',    'linear-gradient(135deg,#15803d,#22c55e)','#fff'],
                ['📦 My Orders',  '/orders',  'linear-gradient(135deg,#6366f1,#8b5cf6)','#fff'],
                ['❤️ Wishlist',   '/wishlist','linear-gradient(135deg,#db2777,#ec4899)','#fff'],
              ].map(([l,p,bg,c]) => (
                <Link key={l} to={p}
                  style={{ padding:'14px', borderRadius:12, background:bg, color:c, fontWeight:700, fontSize:14, textDecoration:'none', textAlign:'center', fontFamily:"'Plus Jakarta Sans',sans-serif", transition:'all .2s', display:'block' }}
            ...
            {['admin','manager'].includes(user?.role) && lowStock.length > 0 && (
