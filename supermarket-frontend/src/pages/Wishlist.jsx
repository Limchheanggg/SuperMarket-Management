    ...
    <div className="page-enter">
      ...
      <div className="container" style={{ padding:'36px 24px' }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:24 }}>
          ...
          {wishlist.length > 0 && (
            <button onClick={() => { setWishlist([]); toast('Wishlist cleared') }}
              style={{ padding:'8px 16px', borderRadius:8, border:'1.5px solid #fecaca', background:'#fef2f2', color:'#dc2626', fontWeight:700, fontSize:13, cursor:'pointer', fontFamily:"'Plus Jakarta Sans',sans-serif" }}>
              Clear All
            </button>
          )}
        </div>
            ...
            {wishlist.map(p => (
              <div key={p.Product_ID} style={{ background:'#fff', border:'1.5px solid #e5e7eb', borderRadius:16, overflow:'hidden', transition:'all .25s', position:'relative' }}
                ...
                <button onClick={() => removeFromWishlist(p.Product_ID)}
                  style={{ position:'absolute', top:8, right:8, zIndex:2, background:'#fee2e2', border:'none', borderRadius:'50%', width:28, height:28, cursor:'pointer', fontSize:14, display:'flex', alignItems:'center', justifyContent:'center', color:'#dc2626' }}>
                  ✕
                </button>
                <Link to={`/shop/${p.Product_ID}`} style={{ display:'flex', height:160, background:'#f0fdf4', alignItems:'center', justifyContent:'center', fontSize:68, textDecoration:'none' }}>
                  {p.Product_Image ? <img src={p.Product_Image} alt={p.Name} style={{ width:'100%', height:'100%', objectFit:'cover' }} /> : (EMOJIS[p.Category_Name] || '🛒')}
                </Link>
                <div style={{ padding:'14px 15px' }}>
                  ...
                  <button onClick={() => moveToCart(p)}
                    style={{ width:'100%', background:'linear-gradient(135deg,#15803d,#22c55e)', color:'#fff', border:'none', borderRadius:8, padding:'9px', fontWeight:700, fontSize:13, cursor:'pointer', fontFamily:"'Plus Jakarta Sans',sans-serif" }}>
                    🛒 Move to Cart
                  </button>
                </div>
              </div>
