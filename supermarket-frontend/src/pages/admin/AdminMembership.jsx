...
export default function AdminMembership() {
  ...
  const fetchAll = async () => {
    setLoading(true)
    try {
      const [mRes, cRes, cpRes] = await Promise.all([
        API.get('/api/membership/'),
        API.get('/api/membership/customers'),
        API.get('/api/coupons/').catch(() => ({ data:[] })),
      ])
      setMembers(mRes.data  || [])
      setCustomers(cRes.data || [])
      setCoupons(cpRes.data  || [])
    } catch(e) {
      toast.error('Failed to load data: ' + (e.response?.data?.detail || e.message))
      setMembers([])
    }
    setLoading(false)
  }
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      setShowModal(false); setSelectedUser(''); setCustomerSearch(''); fetchAll()
  ...
  const handleSaveCoupon = async () => {
    ...
    try {
      ...
      setCouponModal(null); setCouponForm(EMPTY_COUPON); fetchAll()
    } catch(e) { toast.error(e.response?.data?.detail || 'Error') }
    finally { setSaving(false) }
  }
  ...
  const handleDeleteCoupon = async (id) => {
    ...
    try { await API.delete(`/api/coupons/${id}`); toast.success('Deleted'); fetchAll() }
    catch { toast.error('Failed') }
  }
  ...
  const daysLeft  = (d) => !d ? 0 : Math.ceil((new Date(d) - new Date()) / (1000*60*60*24))
  ...
  return (
    <div style={{ padding:28, fontFamily:"'Plus Jakarta Sans',sans-serif" }}>
      ...
      <div style={{ display:'flex', gap:4, marginBottom:20, background:'#f1f5f9', borderRadius:12, padding:4, width:'fit-content' }}>
        {[['members','⭐ Members'],['coupons','🎟️ Coupons']].map(([key,label]) => (
            ...
            <table style={{ width:'100%', borderCollapse:'collapse', fontSize:13 }}>
              ...
              <tbody>
                {coupons.length === 0 ? (
                  ...
                  return (
                    <tr key={c.Coupon_ID} style={{ borderBottom:'1px solid #f8fafc' }}
                      onMouseEnter={e=>e.currentTarget.style.background='#fafafa'}
                      onMouseLeave={e=>e.currentTarget.style.background='transparent'}>
                      <td style={{ padding:'12px 14px' }}>
                        <span style={{ fontFamily:'monospace', fontWeight:800, fontSize:13, color:'#6366f1', background:'#eef2ff', padding:'3px 8px', borderRadius:6 }}>
                          {c.Code}
                        </span>
                      </td>
                      <td style={{ padding:'12px 14px', color:'#374151', maxWidth:160, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>
                        {c.Description}
                      </td>
                      <td style={{ padding:'12px 14px' }}>
                        ...
                        <div style={{ fontSize:10, color:'#94a3b8' }}>{c.Discount_Type}</div>
                      </td>
                      <td style={{ padding:'12px 14px', color:'#64748b' }}>{c.Min_Purchase > 0 ? `$${c.Min_Purchase}` : 'No min'}</td>
                      <td style={{ padding:'12px 14px' }}>
                        <span style={{ padding:'3px 10px', borderRadius:99, fontSize:11, fontWeight:700, background:tc.bg, color:tc.color, border:`1px solid ${tc.border}` }}>
                          {tc.icon} {c.Tier_Required}+
                        </span>
                      </td>
                      <td style={{ padding:'12px 14px' }}>
                        <div style={{ fontSize:12, color:'#374151', fontWeight:600 }}>{c.Uses_Count}/{c.Uses_Limit}</div>
                        <div style={{ height:4, background:'#e5e7eb', borderRadius:99, marginTop:4, overflow:'hidden' }}>
                          <div style={{ height:'100%', width:`${usagePct}%`, background:'#6366f1', borderRadius:99 }} />
                        </div>
                      </td>
                      <td style={{ padding:'12px 14px' }}>
                        <div style={{ fontSize:12, fontWeight:600, color: expired ? '#dc2626' : days <= 7 ? '#d97706' : '#16a34a' }}>
                      ...
                      <td style={{ padding:'12px 14px' }}>
                        <div style={{ display:'flex', gap:5 }}>
                          <button onClick={() => { setCouponForm({...c, Expiry_Date: String(c.Expiry_Date).slice(0,10)}); setCouponModal(c.Coupon_ID) }}
        ...
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.45)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:1000, padding:20 }}
          ...
          <div style={{ background:'#fff', borderRadius:18, padding:32, width:'100%', maxWidth:500, boxShadow:'0 8px 40px rgba(0,0,0,.15)', maxHeight:'90vh', overflowY:'auto' }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:14 }}>
              <div>
                <label style={{ fontSize:13, fontWeight:700, display:'block', marginBottom:6, color:'#374151' }}>Coupon Code *</label>
              ...
              <div>
                <label style={{ fontSize:13, fontWeight:700, display:'block', marginBottom:6, color:'#374151' }}>Usage Limit</label>
              ...
              <div>
                <label style={{ fontSize:13, fontWeight:700, display:'block', marginBottom:6, color:'#374151' }}>Discount Type</label>
              ...
              <div>
                <label style={{ fontSize:13, fontWeight:700, display:'block', marginBottom:6, color:'#374151' }}>Min Purchase ($)</label>
              ...
              <div>
                <label style={{ fontSize:13, fontWeight:700, display:'block', marginBottom:6, color:'#374151' }}>Tier Required</label>
              ...
              <div style={{ gridColumn:'1/-1' }}>
                <label style={{ fontSize:13, fontWeight:700, display:'block', marginBottom:6, color:'#374151' }}>Expiry Date *</label>
                <input type="date" value={couponForm.Expiry_Date}
                  onChange={e => setCouponForm({...couponForm, Expiry_Date:e.target.value})}
                  style={{ width:'100%', padding:'10px 14px', borderRadius:9, border:'1.5px solid #e5e7eb', fontSize:14, outline:'none', boxSizing:'border-box' }} />
              </div>
              <div style={{ gridColumn:'1/-1' }}>
                <label style={{ fontSize:13, fontWeight:700, display:'block', marginBottom:6, color:'#374151' }}>Description</label>
                <input value={couponForm.Description}
                  onChange={e => setCouponForm({...couponForm, Description:e.target.value})}
                  placeholder="e.g. Gold member 20% off"
                  style={{ width:'100%', padding:'10px 14px', borderRadius:9, border:'1.5px solid #e5e7eb', fontSize:14, outline:'none', boxSizing:'border-box' }} />
              </div>
            </div>
            {/* Preview */}
            <div style={{ margin:'18px 0', padding:'14px 16px', background: TIER_COLORS[couponForm.Tier_Required]?.bg||'#f8fafc', borderRadius:12, border:`1.5px solid ${TIER_COLORS[couponForm.Tier_Required]?.border||'#e5e7eb'}`, display:'flex', justifyContent:'space-between', alignItems:'center' }}>
              <div>
                <div style={{ fontFamily:'monospace', fontWeight:800, fontSize:16, color:'#6366f1' }}>{couponForm.Code||'CODE'}</div>
                <div style={{ fontSize:12, color:'#64748b', marginTop:2 }}>{couponForm.Description||'No description'}</div>
              </div>
              <div style={{ textAlign:'right' }}>
                ...
                <div style={{ fontSize:11, color:'#94a3b8' }}>Expires {couponForm.Expiry_Date||'—'}</div>
              </div>
            </div>
            <div style={{ display:'flex', gap:10 }}>
